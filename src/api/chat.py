...
from src.schemas.chat import ChatCreate, ChatOut, UpdateChatTitle
from src.schemas.message import MessageCreate, MessageOut
from src.api.dependencies.chat_dependencies import get_chat_service
from src.services.chat_service import ChatService
from src.api.dependencies.auth import get_current_user
...
# ────────────────────────────────────────────────
# 1. Create new empty chat (optional / explicit)
# ────────────────────────────────────────────────
@router.post(
    "/",
    response_model=ChatOut,
...
async def create_chat(
    ...
    """Create a new chat for the current user (rarely needed – most chats auto-created)."""
    return await service.create_chat(
        user_id=current_user.id,
        title=chat_data.title,
    )
...
# ────────────────────────────────────────────────
# 2. List user's chats (paginated)
# ────────────────────────────────────────────────
@router.get(
    "/",
    response_model=List[ChatOut],
    summary="Get list of the current user's chats",
)
async def get_user_chats(
    skip: int = Query(0, ge=0, description="Skip N chats"),
    limit: int = Query(50, ge=1, le=200, description="Max number of chats to return"),
    current_user: User = Depends(get_current_user),
    service: ChatService = Depends(get_chat_service),
):
    chats = await service.get_user_chats(current_user.id)
    # Simple client-side pagination (upgrade to DB-level later if needed)
    return chats[skip : skip + limit]


# ────────────────────────────────────────────────
# 3. Send message – smart endpoint (auto-create if chat_id is None)
# ────────────────────────────────────────────────
@router.post(
    "/messages",
    response_model=List[MessageOut],
    summary="Send a message (auto-creates chat if chat_id is not provided)",
)
async def send_message(
    ...
    """
    Send a message with the following behavior:
    - If chat_id is provided → continue that chat (must exist and belong to user)
    - If chat_id is NOT provided (None or omitted) → automatically create a new chat
    - If create_new=true → force creation of a new chat even if user has existing ones
    Returns both the user message and the assistant's response.
    """
    if not current_user or not current_user.id:
        raise HTTPException(status_code=401, detail="User not authenticated")

    _, messages = await service.send_message(
        user_id=current_user.id,
        content=request.content,
        chat_id=request.chat_id,       # can be None → auto-create
        create_new=request.create_new, # optional force new
    )
    return messages


# ────────────────────────────────────────────────
# 4. Get messages of a specific chat
# ────────────────────────────────────────────────
@router.get(
    "/{chat_id}/messages",
    response_model=List[MessageOut],
